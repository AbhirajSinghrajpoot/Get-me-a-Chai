...
const Dashboard = () => {
    ...
    const getData = useCallback(async () => {
        // ✅ Use session.user.username (not .name) — set by our NextAuth session callback
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        // ✅ Use session.user.username as the lookup key
        const username = session?.user?.username
        if (!username) return
    ...
    return (
        <>
            {/* ✅ Single ToastContainer — removed the duplicate bare one */}
            <ToastContainer
                position="top-right"
                autoClose={5000}
                hideProgressBar={false}
